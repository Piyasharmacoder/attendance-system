...
export default function Dashboard() {
  ...
  const { data, isLoading, error } = useGetDashboardQuery(userInfo?.role, {
    skip: !userInfo?.role,
  });
  ...
  // Clean formatting (max 4 decimals, no ugly float bug)
  const formattedHours = parseFloat(totalWorkedHours.toFixed(4));
  const targetProgress = Math.max(
    0,
    Math.min(100, Math.round((formattedHours / weekTargetHours) * 100)),
  );
      ...
      <div className="mt-12 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white rounded-3xl p-8 shadow-sm border border-green-50">
          <h2 className="text-xl font-bold mb-2 flex items-center gap-2">
            <span className="p-2 bg-green-100 rounded-lg">📈</span> Weekly
            Activity
          </h2>
            ...
            <div className="rounded-2xl border border-sky-100 bg-sky-50/50 p-4">
              ...
              <div className="mt-3 flex items-end justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Based on present days this week
                  </p>
                </div>
        ...
        <div className="bg-gradient-to-tr from-green-600 to-teal-700 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden">
          <div className="relative z-10">
            ...
            <p className="text-green-100 text-sm leading-relaxed">
              New attendance policy is live. Please ensure your GPS is enabled
              while punching in.
            </p>
    ...
    <div className="group bg-white p-1 rounded-[2.5rem] shadow-sm hover:shadow-xl transition-all duration-300 border border-green-50 hover:-translate-y-2">
      <div className="bg-white rounded-[2.2rem] p-6 h-full">
        <div className="flex justify-between items-start mb-4">
          <div
            className={`${color} w-14 h-14 rounded-2xl flex items-center justify-center text-2xl shadow-lg group-hover:rotate-6 transition-transform`}
          >
            {icon}
          </div>
